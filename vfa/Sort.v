...
Proof.
  ...
  - bdestruct (x <=? a).
    * assert ((x <=? a) = true). apply Nat.leb_le. auto.
      simpl. rewrite -> H0. apply Permutation_refl.
...
Proof.
  ...
  - simpl. constructor.
  - bdestruct (a <=? a0).
  * assert ((a <=? a0) = true). apply Nat.leb_le. auto.
    simpl. rewrite -> H1.
    constructor. auto. auto.
  * assert ((a <=? a0) = false). apply leb_correct_conv. auto.
    simpl. rewrite -> H1. 
    destruct l.
    -- simpl. constructor. omega. constructor.
    -- bdestruct (a <=? n).
       ** assert ((a <=? n) = true). apply Nat.leb_le. auto.
          simpl. rewrite -> H3. constructor. omega. constructor. auto.
          inversion H. auto.
       ** assert ((a <=? n) = false). apply leb_correct_conv. auto.
          simpl. rewrite -> H3. 
          inv H. constructor. auto.
          replace (n :: insert a l) with (insert a (n :: l)).
          --- auto.
...
unfold sorted'. intros. generalize dependent i. generalize dependent j.
induction H; intros.
- destruct H0. inv H0.
- destruct H0. inv H0. inv H. inv H2.
- destruct H1. destruct i, j.
  -- inversion H1.
  -- replace (nth (S j) (x :: y :: l) 0) with (nth j (y :: l) 0).
     * destruct j.
        **  simpl. auto.
        ** eapply le_trans. 
           2 : apply IHsorted with (i:=0). simpl. auto.
           split. omega. simpl. simpl in H2. omega.
     * auto.    
  -- replace (nth (S i) (x :: y :: l) 0) with (nth i (y :: l) 0).
     * inv H1.
     * auto.
  -- replace (nth (S j) (x :: y :: l) 0) with (nth j (y :: l) 0).
     * replace (nth (S i) (x :: y :: l) 0) with (nth i (y :: l) 0).
       ** apply IHsorted. split. omega. simpl. simpl in H2. omega.
       ** auto.
     * auto.
Qed.
...
Proof.
  ...
  - destruct al.
    ...
    -- constructor.
       * unfold sorted' in H.
         specialize (H 0 1). simpl in H.
         apply H. split. auto. omega.
       * apply IHal. unfold sorted'. intros.
         unfold sorted' in H. 
         replace (nth i (n :: al) 0) with (nth (S i) (a :: n :: al) 0).
         --- replace (nth j (n :: al) 0) with (nth (S j) (a :: n :: al) 0).
           **  apply H. destruct H0. split. omega. simpl. simpl in H1. omega.
           ** auto.
         ---  auto.
Qed.
(** [] *)
...
Proof.
  intros. induction al; split; intros.
  - inversion H0.
  - auto.
  - inv H. destruct i.
    * simpl. auto.
    * simpl. apply IHal. auto. simpl in H0. omega.
  - constructor. 
