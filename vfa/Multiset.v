...
Proof.
  ...
  - auto.
  - bdestruct (x <=? a).
    * assert ((x <=? a) = true). apply Nat.leb_le. auto. simpl. rewrite -> H0. auto.
    * assert ((x <=? a) = false). apply leb_correct_conv. auto.
      simpl. rewrite -> H0. simpl. rewrite <- IHl. simpl.
      rewrite -> union_assoc. symmetry. rewrite -> union_assoc.
      f_equal.
      apply union_comm.
Qed.
(** [] *)
...
Proof.
  ...
  - auto.
  - simpl. f_equal. auto.
  - simpl. rewrite -> union_assoc. symmetry. rewrite -> union_assoc.
...
Proof.
  ...
  - subst. 
    bdestruct (x =? v).
    -- subst. assert ((v =? v) = true). apply Nat.eqb_refl. unfold multiset_delete.
       rewrite -> H. unfold union. unfold singleton. rewrite -> H. omega.
    -- assert ((x =? v) = false). apply Nat.eqb_neq. auto. 
       unfold multiset_delete. rewrite -> H0.  unfold union. unfold singleton.
       rewrite ->  H0. omega.
  - simpl. assert ((v =? a) = false). apply Nat.eqb_neq. auto. 
    bdestruct (x =? v).
    -- subst. assert ((v =? v) = true). apply Nat.eqb_refl. unfold multiset_delete.
       rewrite -> H1. unfold union. unfold singleton. rewrite -> H0. simpl. 
       rewrite -> IHal. unfold multiset_delete. rewrite -> H1. auto.
    -- assert ((x =? v) = false). apply Nat.eqb_neq. auto. 
       unfold multiset_delete. rewrite -> H2.
       unfold union. f_equal. rewrite -> IHal.
       unfold multiset_delete. rewrite -> H2. auto.
Qed.
(** [] *)
...
Proof.
  intros.
  assert (empty v = (union (singleton v) b) v). rewrite -> H. auto.
  unfold empty in H0. unfold union in H0. unfold singleton in H0.  
  rewrite -> Nat.eqb_refl in H0. omega.
Qed.
(** [] *)
...
Proof.
  ...
  - bdestruct (a =? a0).
    ...
    -- apply in_cons. apply IHbl.
       simpl in H. unfold union in H. unfold singleton in H.
       assert ((a =? a0) = false). apply Nat.eqb_neq. auto.
...
Proof.
  ...
  - inv H.
  - inv H.
   -- constructor. simpl. rewrite -> Nat.eqb_refl. auto.
   -- bdestruct (a =? a0).
     *  subst. simpl. rewrite -> Nat.eqb_refl. auto.
     * assert ((a0 =? a) = false). apply Nat.eqb_neq. auto.
       simpl. rewrite -> H1.
       eapply Permutation_trans. Search Permutation.
       apply perm_swap. apply perm_skip. apply IHbl. auto.
Qed.
(** [] *)
...
Proof.
  ...
  (** From this point on, you don't need induction.
   ...
   assert (In a cl).
   - apply contents_in. 
     rewrite <- H. simpl. unfold union. unfold singleton. rewrite -> Nat.eqb_refl. omega.
   - assert (Permutation (a :: list_delete cl a) cl). apply in_perm_delete. auto.
     apply perm_trans with (l' := (a :: (list_delete cl a))).
     -- apply perm_skip. apply IHal.  
        rewrite -> delete_contents.
        extensionality x.
        assert (contents (a :: al) x = contents cl x). rewrite -> H. auto.
        bdestruct (x =? a).
        *  subst. unfold multiset_delete. rewrite ->  Nat.eqb_refl.
           rewrite <- H2. simpl. unfold union. unfold singleton. rewrite ->  Nat.eqb_refl.
           omega.
        *  assert ((x =? a) = false). apply Nat.eqb_neq. auto.
           unfold multiset_delete. rewrite -> H4. rewrite <- H2.
           simpl. unfold union. unfold singleton. rewrite -> H4. omega.
     -- auto.
Qed.
