...
Inductive alt_list : Type :=
  | first (f : natlist)
  | second (s : natlist)
  | with_pair (a b : nat)(t : alt_list).

Fixpoint to_alt (l1 l2 : natlist) : alt_list :=
  match l1 with 
  | nil => second l2
  | h1 :: t1 => match l2 with 
                  | nil => first l1
                  | h2 :: t2 => with_pair h1 h2 (to_alt t1 t2)
                  end
  end.
      
Fixpoint from_alt (a : alt_list) : natlist :=
  match a with
  | first f => f
  | second s => s
  | with_pair a b t => a :: b :: (from_alt t)
  end.

Fixpoint alternate (l1 l2 : natlist) : natlist:=
  from_alt (to_alt l1 l2).
...
Lemma peano_refl : forall k : nat, PeanoNat.Nat.eqb k k  = Datatypes.true .
Proof.
    intros.
    induction k.
    { 
...
Theorem bag_theorem : forall (v : nat) (s : bag) , (count v (add v s)) = (S (count v s)) .
Proof.
  intros.
  induction s.
  {
    simpl.
    rewrite -> peano_refl. reflexivity.
  }
  {
     simpl.
     rewrite -> peano_refl.
...
Fixpoint eqblist (l1 l2 : natlist) : bool := 
  ...
  | nil, a :: b => false
  | a :: b, nil => false
  | h1 :: t1, h2 :: t2 => if beq_nat h1 h2 then eqblist t1 t2 else false
  end.  
...
Proof.
   intros.
   assert (forall l : natlist, l = rev (rev l)). 
   {
       symmetry . rewrite -> rev_involutive. reflexivity.
    }
   ...
   {
      rewrite -> H0. symmetry. rewrite -> H0.
      rewrite -> H1.
      reflexivity.
    }
Qed.
