...
Proof.
  ...
  induction n.
  {
    destruct m. simpl. reflexivity. simpl. reflexivity.
  }
  {
    intros.
    destruct m.
    simpl. reflexivity. simpl. apply IHn.
  }
Qed.
(** [] *)
     ...
     destruct m. 
      ...
      }
      {
          ...
          }
          {
              ...
              simpl in H.
              simpl in H0.
              generalize H0. generalize H. apply IHn.
          }
      }
  }
Qed.
...
Lemma filter_head_tr : forall (X : Type) (test : X -> bool)
                             (x y : X) (xs ys : list X),
     test x = true -> filter test (x :: xs) = (y :: ys) ->
     x = y.
Proof.
   intros.
   unfold filter in H0.
...
Lemma filter_head_false : forall (X : Type) (test: X -> bool) (x : X) (l : list X),
