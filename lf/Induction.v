...
Lemma mult_zero : forall k : nat, k * 0 = 0.
Proof.
 induction k.
     {
        simpl. reflexivity.
...
Lemma mult_dist : forall m n : nat, m * S n = m + m * n.
Proof.
   intros.
   induction m.
   {
...
Lemma aux : forall n m a b : nat, n + m + (a + b) = n + a + (m + b).
Proof.
  intros.
  rewrite -> plus_assoc' .
  symmetry.
...
Lemma nat_bin_incr : forall b, bin_to_nat (incr b) = S (bin_to_nat b).
Proof. 
  intros.
  induction b.
  {
...
(*Theorem normalize_works : forall b : bin, (nat_to_bin (bin_to_nat b)) = normalize b.
Proof.
   intros.
   induction b.
   {
